...
function ageText(dateValue){
  if(!dateValue)return "";
  const start=new Date(`${dateValue}T12:00:00`),now=new Date();
...
function cardStatus(profile){
  if(profile.relationship==="Wild Friend"){const last=profile.sightingHistory.at(-1);return last?`Last seen ${formatDate(last,false)}`:"No sightings yet"}
  if(profile.relationship==="Memorial")return "Memory profile";
  const last=profile.feedHistory.at(-1);return last?`Last fed ${formatDate(last,false)}`:"Not fed yet";
}
function renderAnimals(profiles){
  ...
  $("#animalGrid").innerHTML=filtered.length?filtered.map(profile=>`
    <button class="grid-card" data-animal="${profile.id}">
      ...
      <div class="grid-status"><span class="grid-status-dot ${healthClass(profile.health)}"></span><span>${esc(ageText(profile.birthday)||cardStatus(profile))}</span></div>
...
function renderMemoryOfDay(profiles){
  const candidates=[];profiles.forEach(profile=>(profile.gallery||[]).forEach(photo=>candidates.push({profile,photo})));
  if(!candidates.length){$("#memoryCard").hidden=true;return}
  const favourites=candidates.filter(x=>x.photo.favourite),pool=favourites.length?favourites:candidates;
  const chosen=pool[Math.floor(Date.now()/86400000)%pool.length];
  $("#memoryCard").hidden=false;
  $("#memoryTitle").textContent=chosen.photo.favourite?"A favourite memory":"A moment from your world";
  $("#memoryContent").innerHTML=`<div class="memory-photo"><img src="${chosen.photo.src}" alt="${esc(chosen.profile.name)}"><div><strong>${esc(chosen.profile.name)}</strong><small>${esc(chosen.photo.caption||"A photo worth keeping.")}</small></div></div>`;
}
...
function updateBuilderSelection(){
  ["#shrinkItem","#growItem","#rotateItem","#duplicateItem","#deleteItem"].forEach(selector=>$(selector).disabled=!selectedBuilderItem);
  $("#builderSelection").innerHTML=selectedBuilderItem?`<span>${selectedBuilderItem.textContent} Selected item</span><small>${Math.round(Number(selectedBuilderItem.dataset.scale)*100)}% size • ${selectedBuilderItem.dataset.rotation}°</small>`:"<span>Nothing selected</span><small>Tap an object inside the habitat to edit it.</small>";
}
function selectBuilderItem(el){
  document.querySelectorAll(".placed-item").forEach(x=>x.classList.remove("selected"));
  selectedBuilderItem=el;el?.classList.add("selected");updateBuilderSelection();
}
function updateItemTransform(el){el.style.transform=`scale(${el.dataset.scale}) rotate(${el.dataset.rotation}deg)`;if(el===selectedBuilderItem)updateBuilderSelection()}
...
$("#deleteItem").onclick=()=>{selectedBuilderItem?.remove();selectedBuilderItem=null;updateBuilderSelection()};
$("#clearHabitat").onclick=()=>{if(confirm("Clear this layout?")){document.querySelectorAll(".placed-item").forEach(x=>x.remove());selectedBuilderItem=null;updateBuilderSelection()}};
