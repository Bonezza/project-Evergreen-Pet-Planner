...
function openFeature(title){
 ...
 if($("#galleryInput"))$("#galleryInput").onchange=async e=>{let f=e.target.files?.[0];if(!f)return;let data=await resize(f),p=getProfiles(),x=p.find(v=>v.id===a.id);x.gallery=x.gallery||[];x.gallery.push(data);saveProfiles(p);$("#featureDialog").close();render();showToast("Photo added 📸")};
}
